...
- (BOOL)shouldArchiveMessage:(XMPPMessage *)message outgoing:(BOOL)isOutgoing xmppStream:(XMPPStream *)_xmppStream
{
    // ini tambahan gue
    NSString *domain = [message from].domain;
    NSString *resource = [message from].resource;
    NSString *myUser = _xmppStream.myJID.user;
    
    if(([domain isEqualToString:@"room.lb1.smilesatme.com"] || [domain isEqualToString:@"group.lb1.smilesatme.com"]) && ([resource isEqualToString:myUser] || resource.length < 1)){
        return NO;
    }
    
    if([domain isEqualToString:@"room.lb1.smilesatme.com"] || [domain isEqualToString:@"group.lb1.smilesatme.com"]){
        ...
        if(m.message.body.length > 0)
            return NO;
    }
    
    // akhir dari tambahan gue

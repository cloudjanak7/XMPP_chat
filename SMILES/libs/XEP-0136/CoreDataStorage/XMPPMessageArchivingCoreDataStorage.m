...
-(NSArray *)getMessageWithJid:(XMPPJID *)contactJid streamJid:(XMPPJID *)streamJid{
        NSEntityDescription *messageEntity = [self messageEntity:[self mainThreadManagedObjectContext]];
        
        NSString *predicateFrmt = @"bareJidStr == %@ AND streamBareJidStr == %@";
        NSPredicate *predicate = [NSPredicate predicateWithFormat:predicateFrmt,
                                  [contactJid bare], [streamJid bare]];
...
-(XMPPMessageArchiving_Message_CoreDataObject *)getLastMessageWithJid:(XMPPJID *)contactJid streamJid:(XMPPJID *)streamJid{
    NSEntityDescription *messageEntity = [self messageEntity:[self mainThreadManagedObjectContext]];
    
    NSPredicate *predicate = [NSPredicate predicateWithFormat:@"streamBareJidStr == %@ AND bareJidStr == %@", [streamJid bare], contactJid.bare];
...
-(XMPPMessageArchiving_Message_CoreDataObject *)getMessageWithID:(NSString *)strID{
    ...
    NSPredicate *predicate = [NSPredicate predicateWithFormat:@"(messageStr CONTAINS[cd] %@)", strID];
...
-(void)insertMessage:(XMPPMessageArchiving_Message_CoreDataObject *)newMessage{
    NSError *error = nil;
    [[self mainThreadManagedObjectContext] save:&error];
}

-(void)deleteMessageForJID:(XMPPJID *)jid streamJid:(XMPPJID *)streamJid{
    NSArray *messages = [self getMessageWithJid:jid streamJid:streamJid];
    
    for(XMPPMessageArchiving_Message_CoreDataObject *obj in messages){
        [[self mainThreadManagedObjectContext] deleteObject:obj];
    }
    NSError *error;
    [[self mainThreadManagedObjectContext] save:&error];;
}
...
-(void)deleteMessageWithID:(NSString *)msgID{
    ...
    XMPPMessageArchiving_Message_CoreDataObject *obj = [self getMessageWithID:msgID];
    [[self mainThreadManagedObjectContext] deleteObject:obj];
    NSError *error;
    [[self mainThreadManagedObjectContext] save:&error];;
}

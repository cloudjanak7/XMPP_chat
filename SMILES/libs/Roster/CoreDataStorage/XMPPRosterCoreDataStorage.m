...
-(XMPPUserCoreDataStorageObject *)myUserForXMPPStream:(XMPPStream *)stream{
    return [self myUserForXMPPStream:stream managedObjectContext:[self mainThreadManagedObjectContext]];
}
                         ...
                         managedObjectContext:(NSManagedObjectContext *)moc
{
	...
	NSPredicate *predicate;
	if (stream == nil || [jid isEqualToJID:stream.myJID options:XMPPJIDCompareBare])
...
- (XMPPUserCoreDataStorageObject *)userForJID:(XMPPJID *)jid
                                   xmppStream:(XMPPStream *)stream
{
	return [self userForJID:jid xmppStream:stream managedObjectContext:[self mainThreadManagedObjectContext]];
}
...
- (NSArray *)XMPPUserForXMPPStream:(XMPPStream *)stream{
	...
	[self executeBlock:^{
		...
		{
			NSPredicate *predicate;
			predicate = [NSPredicate predicateWithFormat:@"streamBareJidStr == %@ AND jidStr  contains[c] '@'",
                         [[self myJIDForXMPPStream:stream] bare]];
